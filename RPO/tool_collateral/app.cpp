...
void get_collateral(vector<uint8_t> quote) {
    ...
    JsonValue collateralValue(json_value_init_object());
    tcf::error::ThrowIf<tcf::error::RuntimeError>(
        !collateralValue.value, "Failed to create enclave collateral serialization object");
    JSON_Object* collateral_json_obj = json_value_get_object(collateralValue);
    JsonSetNumber(collateral_json_obj, "collateral_size", p_collateral_size, "faild to set collateral_size to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "version", version, "faild to set version to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "tee_type", tee_type, "faild to set tee_type to collateral_json_obj");
    ...
    JsonSetStr(collateral_json_obj, "pck_crl_issuer_chain", pck_crl_issuer_chain_base64.c_str(), "faild to set pck_crl_issuer_chain to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "pck_crl_issuer_chain_size", pck_crl_issuer_chain_size, "faild to set pck_crl_issuer_chain_size to collateral_json_obj");
    ...
    JsonSetStr(collateral_json_obj, "root_ca_crl", root_ca_crl_base64.c_str(), "faild to set root_ca_crl to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "root_ca_crl_size", root_ca_crl_size, "faild to set root_ca_crl_size to collateral_json_obj");

    vector<uint8_t> pck_crl_vector(pck_crl, pck_crl + pck_crl_size);
    string pck_crl_base64 = base64_encode(pck_crl_vector);
    JsonSetStr(collateral_json_obj, "pck_crl", pck_crl_base64.c_str(), "faild to set pck_crl to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "pck_crl_size", pck_crl_size, "faild to set pck_crl_size to collateral_json_obj");
    ...
    JsonSetStr(collateral_json_obj, "tcb_info_issuer_chain", tcb_info_issuer_chain_base64.c_str(), "faild to set tcb_info_issuer_chain to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "tcb_info_issuer_chain_size", tcb_info_issuer_chain_size, "faild to set tcb_info_issuer_chain_size to collateral_json_obj");
    
    vector<uint8_t> tcb_info_vector(tcb_info, tcb_info + tcb_info_size);
    string tcb_info_base64 = base64_encode(tcb_info_vector);
    JsonSetStr(collateral_json_obj, "tcb_info", tcb_info_base64.c_str(), "faild to set tcb_info to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "tcb_info_size", tcb_info_size, "faild to set tcb_info_size to collateral_json_obj");
    ...
    JsonSetStr(collateral_json_obj, "qe_identity_issuer_chain", qe_identity_issuer_chain_base64.c_str(), "faild to set qe_identity_issuer_chain to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "qe_identity_issuer_chain_size", qe_identity_issuer_chain_size, "faild to set qe_identity_issuer_chain_size to collateral_json_obj");
    ...
    JsonSetStr(collateral_json_obj, "qe_identity", qe_identity_base64.c_str(), "faild to set qe_identity to collateral_json_obj");
    JsonSetNumber(collateral_json_obj, "qe_identity_size", qe_identity_size, "faild to set qe_identity_size to collateral_json_obj");

    size_t serializedSize = json_serialization_size(collateralValue);
    vector<char> serialized_buffer;
    serialized_buffer.resize(serializedSize);
    JSON_Status jret = json_serialize_to_buffer(collateralValue, &serialized_buffer[0], serializedSize);
    tcf::error::ThrowIf<tcf::error::RuntimeError>(
            jret != JSONSuccess, "collateral serialization failed");
    ...
    fwrite(serialized_buffer.data(), serialized_buffer.size() - 1, 1, fptr0);
    fclose(fptr0);

    // Compute SHA384 of collateral
    computeDegist(serialized_buffer.data(), serialized_buffer.size() - 1);
